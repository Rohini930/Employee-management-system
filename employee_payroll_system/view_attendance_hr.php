...
// When form is submitted, fetch attendance for that employee/month/year
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    ");
    $attStmt->bind_param("iii", $selected_employee, $selected_month, $selected_year);
    $attStmt->execute();
    $res = $attStmt->get_result();
    while ($row = $res->fetch_assoc()) {
        $attendance_records[$row['attendance_date']] = $row['status'];
    }
    $attStmt->close();
}
...
// Calculate how many days in the selected month/year
...
<head>
  <meta charset="UTF-8">
  <title>HR View Attendance</title>
...
<body>
  <div class="container">
    ...
    <?php if ($selected_employee): ?>
      <h3>Attendance for <?= htmlspecialchars(date('F', mktime(0,0,0,$selected_month,1))) ?>
          <?= htmlspecialchars($selected_year) ?></h3>
      <table>
        <tr><th>Date</th><th>Status</th></tr>
        <?php for ($d = 1; $d <= $daysInMonth; $d++):
            $date = sprintf('%04d-%02d-%02d', $selected_year, $selected_month, $d);
            $status = $attendance_records[$date] ?? 'Missed';
        ?>
          <tr>
            <td><?= $date ?></td>
            <td><?= $status ?></td>
